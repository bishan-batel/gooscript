...
namespace goos::ast::expression {
  auto Binary::get_lhs() const -> const Expression & { return lhs; }

  auto Binary::get_rhs() const -> const Expression & { return rhs; }

  auto Binary::get_op() const -> const lexer::Operator & { return op; }

  auto Binary::to_string() const -> WideString {
    return fmt::format(L"({}) {} ({})", lhs->to_string(), lexer::OPERATOR_TO_STR_MAP.at(op), rhs->to_string());
  }

  auto Binary::clone_expr() const -> Box<Expression> {
    return crab::make_box<Binary>(lhs->clone_expr(), op, rhs->clone_expr(), operator_trace);
  }
  ...
  auto Binary::token_trace() const -> TokenTrace {
    return TokenTrace::merge_fold(lhs->token_trace(), rhs->token_trace(), operator_trace);
  }
