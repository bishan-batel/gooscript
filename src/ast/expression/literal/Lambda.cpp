...
namespace goos::ast::expression {
  ...
  auto Lambda::get_body() const -> const Expression & { return body; }

  auto Lambda::clone_expr() const -> Box<Expression> {
    return crab::make_box<Lambda>(params, body->clone_expr(), trace);
  }
  ...
  auto Lambda::operator==(const Statement &statement) const -> bool {
    ...
    if (lambda->params.size() != params.size())
      return false;
    if (*lambda->body != *body)
      return false;

    for (usize i = 0; i < params.size(); i++) {
      if (params[i] != lambda->params[i])
        return false;
    }
    return true;
  }
  ...
  auto Lambda::accept_expr(IVisitor &visitor) const -> Result<std::any, Box<crab::Error>> {
    return visitor.visit_lambda(*this);
  }

  auto Lambda::get_params() const -> const Vec<meta::Identifier> & { return params; }
} // namespace goos::ast::expression
