...
namespace goos::runtime {
  ...
  auto ExternFunction::varargs(const decltype(function) &function) -> RcMut<ExternFunction> {
    return RcMut<ExternFunction>::from_owned_unchecked(
      new ExternFunction{function, 0, ArityType::Varargs}
    );
  }
  ...
  auto ExternFunction::get_type() const -> meta::VariantType {
    return meta::VariantType::FUNCTION;
  }

  auto ExternFunction::get_arity_type() const -> ArityType {
    return arity_type;
  }

  auto ExternFunction::get_arity() const -> usize {
    return arity;
  }
