...
namespace goos::runtime {
  ...
  GString::GString(const meta::Identifier &identifier)
      : text{identifier.get_string_ref_counted()}, cached_hash{identifier.get_hash()} {}

  GString::GString(Rc<WideString> string) : text{std::move(string)}, cached_hash{utils::hash(*this->text)} {}

  auto GString::get() const -> Rc<WideString> { return text; }

  auto GString::to_string() const -> WideString { return *text; }

  auto GString::get_type() const -> meta::VariantType { return meta::VariantType::STRING; }

  auto GString::base_hash() const -> usize { return cached_hash; }

  auto GString::clone() const -> Any { return crab::make_rc_mut<GString>(text); }
  ...
  auto GString::to_json() const -> Box<json::Value> { return json::to_json(text.get()); }
} // namespace goos::runtime
