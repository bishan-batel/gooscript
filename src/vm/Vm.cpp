  ...
  auto Vm::run_instruction(const op::Code code) -> Result<unit> {
    switch (code) {
      ...
      case op::Code::JUMP_IF_FALSE: {
        return pop_as<bool>().map(
          [this](const bool condition) {
            if (not condition) {
            ...
            return first.match(\
              crab::cases{\
                [&]<typename T, typename K>(const T a, const K b) {\
                  return push(Value{a op b});\
                },\
                [&]<typename T, typename K> requires (not requires(const T a, const K b) {\
                  { a op b } -> std::convertible_to<Value>;\
                })(const T, const K) -> Result<unit> {\
                  return vm::make_err<err::UnsupportedOperation>(\
                    name,\
                    std::vector{first.type_name(), second.type_name()}\
                  );\
                }\
              },\
              second\
      ...
      case op::Code::LESS_THAN: { return bin_op("LESS_THAN", <); }
      case op::Code::GREATER_OR_EQUAL_THAN: { return bin_op("GREATER_OR_EQUAL_THAN", >=); }
      case op::Code::LESS_OR_EQUAL_THAN: { return bin_op("LESS_OR_EQUAL_THAN", <=); }
      case op::Code::BIT_AND: { return bin_op("BIT_AND", &); }
      case op::Code::BIT_OR: { return bin_op("BIT_OR", |); }
      case op::Code::BIT_XOR: { return bin_op("BIT_XOR", ^); }
      case op::Code::SHIFT_LEFT: { return bin_op("SHIFT_LEFT", <<); }
      case op::Code::SHIFT_RIGHT: { return bin_op("SHIFT_RIGHT", >>); }
      case op::Code::ADD: { return bin_op("ADD", +); }
      case op::Code::SUBTRACT: { return bin_op("SUBTRACT", -); }
      case op::Code::MULIPLY: { return bin_op("MULIPLY", *); }
      case op::Code::DIVIDE: { return bin_op("DIVIDE", /); }
      case op::Code::MODULO: {
        return crab::fallible<Box<err::Error>>([this] { return pop(); }, [this] { return pop(); }).and_then(
          [this](auto tuple) -> Result<unit> {
            auto [first, second] = tuple;
            return first.match(
              crab::cases{
                ...
                [&]<typename T, typename K>(T, K) -> Result<unit> {
                  return vm::make_err<err::UnsupportedOperation>(
                    "MODULO",
                    std::vector{Value::type_name_of_s<T>, Value::type_name_of_s<K>}
                  );
                }
              },
              second
      ...
      case op::Code::NEGATE: {
        auto popped = pop();
        if (popped.is_err()) return popped.take_err_unchecked();

        return popped.take_unchecked().match(
          [this](const auto n) -> Result<unit> { return this->push(-n); },
          []<typename T> requires (not
            requires(const T n) { { -n } -> std::convertible_to<Value>; })(const T) -> Result<unit> {
            return make_err<err::UnsupportedOperation>(
              "NEGATE",
              std::vector{Value::type_name_of_s<T>}
            );
          }
        );
      }
